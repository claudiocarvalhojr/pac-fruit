...
public class Main {
	...
	{
		String[] options = {"instalar", "jogar", "cancelar"};
		String question = "Você deseja " + options[0] + " ou " + options[1] + " pac-fruit?";
		final int input = JOptionPane.showOptionDialog(null, question, "Pac-fruit", JOptionPane.DEFAULT_OPTION,
				JOptionPane.QUESTION_MESSAGE, new ImageIcon(Main.class.getResource("/main/resources/img/pac-fruit.png")), options, options[0]);
		if (input == 0)
		{
		
			try {
				Instalador.initialize(false, "main/resources/properties/config.properties");
			} catch (IOException e) {
				e.printStackTrace();
			}
			...
			public void keyReleased(final KeyEvent event) {
				if (game.getPlayer() != null && game.getListFruits() != null && !game.getListFruits().isEmpty()) {
					if (event.getKeyCode() == 37 && (game.getPlayer().getLocation().x - moveRL) > 0) {
						game.getPlayer().setLocation(game.getPlayer().getLocation().x - moveRL,
						...
						game.getPlayer().setLocation(game.getPlayer().getLocation().x,
								game.getPlayer().getLocation().y - moveUD);
						collisionFruit(game.getListFruits().size());
				...
				if (event.getKeyCode() == 27) {
					final int input = JOptionPane.showConfirmDialog(panel, "Deseja sair?", "Pac-fruit",
		...
		if (rain != null && game.getPlayer() != null && rain.getLocation().x == game.getPlayer().getLocation().x
			...
			if (game.getPlayer().getLifes() == 0) {
				...
				final int input = JOptionPane.showConfirmDialog(panel,
						...
						"Pac-fruit", JOptionPane.YES_NO_OPTION, JOptionPane.ERROR_MESSAGE);
				if (input == 0) {
					restart();
				} else {
					System.exit(0);
